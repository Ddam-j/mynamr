...
func (a *App) printUsage(fs *flag.FlagSet) {
	...
	_, _ = fmt.Fprintln(a.stderr, "  .\\mynamr.exe \"코요이 코난 최신작 & 프로필 (Koyoi Konan, 小宵こなん (こよいこなん))\"")
	_, _ = fmt.Fprintln(a.stderr, "  mynamr --rule name_alias_bundle \"미야와키 사쿠라 최신곡 & 프로필 (Sakura Miyawaki, 宮脇咲良)\"")
	...
	_, _ = fmt.Fprintln(a.stderr, "  mynamr --rule name_alias_bundle \"코요이 코난 최신작 & 프로필 (Koyoi Konan, 小宵こなん (こよいこなん))\"")
